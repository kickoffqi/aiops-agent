...
def correlate_prom_loki(ctx: IncidentContext, settings: Settings, top_n: int = 5) -> Dict[str, Any]:
    """
    关联逻辑：
    1) 从 Loki rows 统计哪些 pod 报错最多
    2) 取 top N pods
    3) Prometheus 一次性 query：
       - restarts increase by(pod)
       - crashloop waiting by(pod)
    4) 合并成 evidence，写入 ctx.summary / ctx.prom_queries
    """
    pod_stats = _extract_pod_error_stats(ctx)
    if not pod_stats:
        corr = {"status": "no_loki_pods", "pods": []}
        ctx.summary["correlation"] = corr
        return corr

    # top pods by error_count
    ranked = sorted(pod_stats.items(), key=lambda kv: kv[1].get("error_count", 0), reverse=True)
    top = ranked[:top_n]
    top_pods = [p for p, _ in top if p and p != "unknown"]

    if not top_pods:
        corr = {"status": "no_named_pods", "pods": []}
        ctx.summary["correlation"] = corr
        return corr

    prom = build_client(settings)
    #pod_re = _safe_pod_regex(top_pods)
    pod_re = prom_regex_from_list(top_pods)  # top_pods: List[str]
    ...
    q_crashloop_by_pod = (
        "sum by (pod) ("
        f"increase(kube_pod_container_status_waiting_reason{{{selector}}}[{lookback}m])"
        ")"
        #f'sum by (pod) (kube_pod_container_status_waiting_reason'
        #f'{{namespace="{settings.namespace}", pod="{pod_re}", reason="CrashLoopBackOff"}})'
    )
    ...
    for pod, st in top:
        pods_out.append({
            "pod": pod,
            "loki_error_count": st.get("error_count", 0),
            "loki_containers": st.get("containers", {}),
            "loki_samples": st.get("samples", []),
            "prom_restarts_increase": restarts_by_pod.get(pod, 0.0),
            "prom_crashloop_backoff": crashloop_by_pod.get(pod, 0.0),
        })
    ...
    # 可选：把 correlation 用到 triage 的“证据字段”
    # 例如：最高风险 pod
    worst = max(pods_out, key=lambda x: (x["prom_crashloop_backoff"], x["prom_restarts_increase"], x["loki_error_count"]))
